...
function ClicCanvas(evenement) {
  ...
  if (
    sEtatJeu == "intro" &&
    detecterClicObjet(curseurX, curseurY, oBoutonDemarrer) == true //to click on button
  ) {
    sEtatJeu = "jeu";
    oSonMusique.play(); //play sound
  }
  if (
    detecterClicObjet(curseurX, curseurY, oBoutonsReponse[nPositionReponse]) == //to click answers
    true
  ) {
    nPointage += 1;
    ifProblemeCalled = false;
  } else if (detecterClicObjet(curseurX, curseurY, oImageBGReponse) == false) {
...
function afficherIntro() {
  ...
  oContexte.textAlign = "center";
  oContexte.fillText(
    "Trouver la bonne réponse de la question mathématique et clique sur cela.",
    nLargeur / 2,
    200
  );
...
function problemeMath() { //make the mathematical problemes 
...
function melangerSolution() {//mix the order of the answers
...
function afficherFin() {
  if (nVie <= 0) {
    oContexte.fillText(
      "Vous avez perdu... Il y eu des disparaisons des enfants...",
      nLargeur / 2,
      nHauteur / 2
    );
  } else if (nPointage == 10) {
    oContexte.fillText(
      "Vous avez gagné! Vous n'avez plus faim!",
      nLargeur / 2,
      nHauteur / 2
    );
  }
